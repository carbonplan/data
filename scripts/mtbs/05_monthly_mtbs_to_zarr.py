    ...
    with rasterio.open(src_path_year, "r") as src_raster_year:
        ...
        for month in months:
            ...
            with rasterio.open(src_path_month, "r") as src_raster_month:
                ...
                src_band_tmp = src_band_month * (
                    (src_band_year == 3) | (src_band_year == 4)
                ).astype("uint8")
                src_band_tmp[src_band_year == src_nodata] = src_nodata
                src_band_month = src_band_tmp
                ...
                ds.to_zarr(f"{varname}.zarr", mode="w", encoding={varname: {"compressor": Zlib()}})
...
for year in years:
    for month in months:
        ...
        ds = ds.chunk({"x": 1209, "y": 783})
        results.append(ds[varname])

dates = pd.date_range("1984-01", "2018-12", freq="MS")
...
ds.to_zarr(
    ...
    mode="w",
    encoding={"monthly": {"compressor": Zlib()}},
    consolidated=True,
)
