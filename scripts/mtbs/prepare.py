...
def make_dst_band(src_band, src_resolution):
    ...
    dst_transform = dst_transform * \
        Affine.scale(src_resolution/dst_resolution, src_resolution/dst_resolution)
...
def prepare_mtbs(year, resolution):
    ...
    reproject(src_band, dst_band, 
        ...
        resampling=resampling, src_nodata=src_nodata, dst_nodata=src_raster.meta['nodata'])

    meta = src_raster.meta
    meta.update(width=dst_shape[0], height=dst_shape[1], dtype=str(dst_band.dtype),
        crs=dst_crs.to_wkt(), transform=list(dst_transform), nodata=src_raster.meta['nodata'])
